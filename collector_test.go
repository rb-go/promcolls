...
func (c collectorDescribedByCollect) Collect(ch chan<- prometheus.Metric) {
	ch <- c.cnt
	ch <- c.gge
}

func (c collectorDescribedByCollect) Describe(ch chan<- *prometheus.Desc) {
...
func TestDescribeByCollect(t *testing.T) {
	...
	collidingCollector := collectorDescribedByCollect{
		cnt: prometheus.NewCounter(prometheus.CounterOpts{Name: "c2", Help: "help c2"}),
		gge: prometheus.NewGauge(prometheus.GaugeOpts{Name: "g1", Help: "help g1"}),
	}
	inconsistentCollector := collectorDescribedByCollect{
		cnt: prometheus.NewCounter(prometheus.CounterOpts{Name: "c3", Help: "help c3"}),
		gge: prometheus.NewGauge(prometheus.GaugeOpts{Name: "c3", Help: "help inconsistent"}),
	}
